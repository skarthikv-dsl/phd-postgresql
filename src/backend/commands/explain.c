			...
			ExplainState *es)
{
	...
	if (es->format == EXPLAIN_FORMAT_TEXT)
	{
		...
		//Changed to sname from pname: Srinivas
		appendStringInfoString(es->str, sname);
		es->indent++;
	}
	else
	{
...
if(es->format == EXPLAIN_FORMAT_TEXT){
	switch (nodeTag(plan))
	{
		...
		case T_NestLoop:
			if(nodeTag(innerPlan(plan)) == T_IndexScan){
				ancestors = lcons(planstate, ancestors);
			show_upper_qual(((IndexScan *) innerPlan(plan))->indexqualorig,
							"spill", planstate->righttree, ancestors, es);
			ancestors = list_delete_first(ancestors);

			}

			else if(nodeTag(innerPlan(plan)) == T_BitmapHeapScan){
				ancestors = lcons(planstate, ancestors);
					show_upper_qual(((BitmapHeapScan *) innerPlan(plan))->bitmapqualorig,
									"spill", planstate->righttree, ancestors, es);
					ancestors = list_delete_first(ancestors);
			}
			if(nodeTag(innerPlan(plan)) == T_IndexOnlyScan){
							ancestors = lcons(planstate, ancestors);
						show_upper_qual(((IndexOnlyScan *) innerPlan(plan))->indexqual,
										"spill", planstate->righttree, ancestors, es);
						ancestors = list_delete_first(ancestors);
		...
		case T_MergeJoin:
			...
			if (((MergeJoin *) plan)->join.joinqual)
				show_instrumentation_count("Rows Removed by Join Filter", 1,
										   planstate, es);
			show_upper_qual(plan->qual, "spill", planstate, ancestors, es);
			if (plan->qual)
...
// Srinivas: original version has the following two lines uncommented
...
//						appendStringInfoString(es->str, " Join");
		...
		planstate->instrument && planstate->instrument->nloops > 0)
	{
		...
		else
		{
			if(show_actual_cost)
			{
				ExplainPropertyFloat("Actual Startup Cost", startupCost, 3, es);
				ExplainPropertyFloat("Actual Total Cost", totalCost, 3, es);
				...
				ExplainState *es)
{
	...
	{
		case EXPLAIN_FORMAT_TEXT:
			if(qlabel && strlen(qlabel) == 5 && qlabel[0]=='s' && qlabel[1]=='p' && qlabel[2]=='i' && qlabel[3]=='l' && qlabel[4]=='l'){
				appendStringInfo(es->str, " %s",  value);
				break;
			}
