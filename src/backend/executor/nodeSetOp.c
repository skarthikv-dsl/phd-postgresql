...
static TupleTableSlot *setop_retrieve_direct(SetOpState *setopstate, double *subplan_tuples);
...
ExecSetOp(SetOpState *node, double *subplan_tuples)
{
	...
	if (plannode->strategy == SETOP_HASHED)
	{
		...
		return setop_retrieve_direct(node, subplan_tuples); /* Modified on 12/05/2015 for QUEST */
}
...
setop_retrieve_direct(SetOpState *setopstate, double *subplan_tuples) /* Modified on 12/05/2015 for QUEST */
{
	SetOp	   *node = (SetOp *) setopstate->ps.plan;
	PlanState  *outerPlan;
