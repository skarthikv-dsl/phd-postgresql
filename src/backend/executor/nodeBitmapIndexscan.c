...
MultiExecBitmapIndexScan(BitmapIndexScanState *node)
{
	...
	while (doscan)
	{
		...
		/* <Added on 12/05/2015 for QUEST> */
		node->ss.ps.instrument->totaltuples = nTuples;
		#ifdef PARTIAL_EXEC
			#ifndef REQUESTMODEL
				exec_cost_bitmap_index_scan(node, nTuples - nTuples_prev);
			#else
				exec_cost_bitmap_index_scan_rq(node);
			#endif
		#endif

		/*nTuples_prev holds value of nTuples seen in last cycle*/
		nTuples_prev=nTuples;
		#ifdef PARTIAL_EXEC
...
ExecInitBitmapIndexScan(BitmapIndexScan *node, EState *estate, int eflags)
{
	...
	/* <Added on 12/05/2015 for QUEST> */
	indexstate->loop_count=node->loop_count;
//	indexstate->index_pages=node->index_pages;
	indexstate->index_correlation=node->index_correlation;
	indexstate->spc_random_page_cost=node->spc_random_page_cost;
	indexstate->qual_arg_cost=node->qual_arg_cost;
	indexstate->per_tuple_cost=node->per_tuple_cost;
	indexstate->num_sa_scans=node->num_sa_scans;
