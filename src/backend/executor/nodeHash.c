...
MultiExecHash(HashState *node)
{
	...
	for (;;)
	{
		...
		if (TupIsNull(slot))
		{
			...
			tuples=0.0;
				...
				#ifndef REQUESTMODEL
					exec_cost_multiExecHash(node,tuples);
				#else
					exec_cost_multiExecHash_rq(node);
				#endif
	...
	/* <Added on 12/05/2015 for QUEST> */
	//	node->ps.instrument->totaltuples = hashtable->totalTuples;

	//	printf("\nPipeline(H): ");
	//	print_relids(node->ps.plan->relids);
	//	printf("\n");
 ...
 * for the latter.
 */
/* <Arguments modified on 12/05/2015 for QUEST> */
bool
ExecScanHashBucket(HashJoinState *hjstate,
				   ExprContext *econtext, double *hash_qual_tuples_comp)
{
