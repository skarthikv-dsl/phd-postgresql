...
%type <list>	stmtblock stmtmulti
				...
				clauses_list opt_clauses_list select_list opt_select_list /*added for selectivity injection @kuntal*/

%type <str> clause //@kuntal
...
%token <keyword> ABORT_P ABSOLUTE_P ACCESS ACTION ADD_P ADMIN AFTER
	...
	SAVEPOINT SCHEMA SCROLL SEARCH SECOND_P SECURITY SELECT SELECTIVITY SEQUENCE SEQUENCES
...
ExplainStmt:
		...
		| EXPLAIN analyze_keyword opt_verbose selectivity_keyword '(' a_expr ')' '(' opt_select_list ')'  ExplainableStmt /*For Selectivity Injection @kuntal*/
				{
					ExplainStmt *n = makeNode(ExplainStmt);
					n->query = $11;
					n->options = list_make1(makeDefElem("analyze", NULL));
		...
		| EXPLAIN selectivity_keyword '(' a_expr ')' '(' opt_select_list ')'  ExplainableStmt /*For Selectivity Injection @kuntal*/
				{
					ExplainStmt *n = makeNode(ExplainStmt);
					n->query = $9;
					n->options = NIL;
		...
		| EXPLAIN '(' explain_option_list ')' selectivity_keyword '(' a_expr ')' '(' opt_select_list ')' ExplainableStmt
				{
					ExplainStmt *n = makeNode(ExplainStmt);
					n->query = $12;
					n->options = $3;
...
/*For selectivity Injection @kuntal*/	
...
clauses_list: clause         { $$ = list_make1(makeString($1)); }
                  | clauses_list ',' clause
                             { $$ = lappend($1, makeString($3)); }
;
...
select_list: FCONST         { $$ = list_make1(makeFloat($1));  }
                  | select_list ',' FCONST
                            { $$ = lappend($1, makeFloat($3)); }
;
