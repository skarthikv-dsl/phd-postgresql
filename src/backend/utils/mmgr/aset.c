					  ...
					  Size maxBlockSize)
{
	...
	if (initBlockSize < 1024)   // Srinivas changed from 1024 to 2048
		initBlockSize = 1024;
...
AllocSetFree(MemoryContext context, void *pointer)
{
	...
	if (chunk->requested_size < chunk->size)
		if (!sentinel_ok(pointer, chunk->requested_size))
			if (((char *) pointer)[chunk->requested_size] != 0x7E){						/* < Added for QUEST > */
	...
	for (block = set->blocks; block != NULL; block = block->next)
	{
		...
		while (bpoz < block->freeptr)
		{
			...
			if (chsize > set->allocChunkLimit &&
				...
				elog(WARNING, "problem in alloc set %s: bad single-chunk %p in block %p : %d > %d and %l != %l",
					 name, chunk, block, chsize, set->allocChunkLimit, (chsize + ALLOC_CHUNKHDRSZ), blk_used);
