				   ...
				   SpecialJoinInfo *sjinfo)
{
	...
	bool inject_join_selectivity = false; //kuntal
	...
	{
		rinfo = (RestrictInfo *) clause;
//		printf("%d %f\n",rinfo->clause_relids->words[0],rinfo->injectedSelectivity);
		/*
		...
		//Commented by kuntal - We don't want to cache the selectivity estimation for a query
		#ifndef JOIN_SELECTIVITY_INJECTION
			if (varRelid == 0 ||
	...
	else if (is_opclause(clause) || IsA(clause, DistinctExpr))
	{
		...
		if (treat_as_join_clause(clause, rinfo, varRelid, sjinfo))
		{
			...
			s1 = join_selectivity(root, opno,
								  ...
								  jointype,
								  sjinfo);
			#ifdef JOIN_SELECTIVITY_INJECTION
				inject_join_selectivity = true;
			#endif
		}
		else
		{
			...
			s1 = restriction_selectivity(root, opno,
										 ...
										 varRelid);
			#ifdef JOIN_SELECTIVITY_INJECTION
			if(!root->indexUnique_EqualityOperator) {
				inject_join_selectivity = true;
			}
			#endif
		}
	...
	if(inject_join_selectivity) {
		if (rinfo!=NULL)
				{
					ListCell *l1,*l2;
					forboth(l1, (List *) root->parse->clausesel, l2, root->parse->selec)
					{
						cnt ++;
					}
					...
					forboth(l1, (List *) root->parse->clausesel, l2, root->parse->selec)
					{
						...
						Expr	   *clause1 = (Expr *) lfirst(l1);

						if ((is_opclause(clause1) && list_length(((OpExpr *) clause1)->args) == 2))
						{
							Relids left_relids = pull_varnos(get_leftop(clause1));

							if(get_leftop(clause1) == clause)
								printf("true\n");
							Index rti;
							float js_mult = 1;
							for (rti = 1; rti < root->simple_rel_array_size; rti++)
							{
								...
								if(rel->relids->words[0]==left_relids->words[0]) {
									js_mult = rel->tuples/rel->rows;

									char buffer [33];
									//sprintf(buffer, "%f", js_mult);
								//	printf("rel_tuples= %f, js_mult= %f \n", rel->tuples, js_mult);
								//	printf("js_mult changed for relid %d \n",rel->relids->words[0]);
								}
							}


							Relids right_relids = pull_varnos(get_rightop(clause1));

							Relids clause_relids = bms_union(left_relids,right_relids);
							if(rinfo->clause_relids->words[0]==clause_relids->words[0]) {
								float iselec = atof(((Value *) lfirst(l2))->val.str);
							if(bms_equal(clause_relids,left_relids) || bms_equal(clause_relids,right_relids)) {
								    //base clause scenario

									if((itr == base_rel_cnt) || (cnt == 1))
										s1 = iselec;
...
//									else if (cnt == itr){
//										s1 = s1*iselec;
//										s1 = pow(s1, 1.0/(cnt*1.0));
//									}
//									else{
//										s1 = s1*iselec;
//									}
								    //printf("baseId \tInjected Selectivity \tNew Selectivity\n");
								    //printf("%d \t%f \t%f  \n",rinfo->clause_relids->words[0],iselec,s1);

								}
								else{
								    //printf("js_mult = %f", js_mult);
								    s1 = s1*iselec*js_mult;
								}
								//printf("js_mult = %f", js_mult);
								//s1 = s1*iselec*js_mult;
							//	printf("JoinId \tInjected Selectivity \tJS_Multipler \tNew Selectivity\n");
							//	printf("%d \t%f \t%f \t%f \n",rinfo->clause_relids->words[0],iselec,js_mult,s1);
							}
						}
						else if((IsA(clause, ScalarArrayOpExpr) && list_length(((ScalarArrayOpExpr *) clause1)->args) == 2 )){
							// this case to handle base sel injection for 'in' clauses
							const ScalarArrayOpExpr *expr = (const ScalarArrayOpExpr *) clause;
							Relids left_relids;
							if (expr->args != NIL)
								left_relids = pull_varnos(linitial(expr->args));

							Relids right_relids;
							if (list_length(expr->args) >= 2)
									right_relids = pull_varnos(lsecond(expr->args));
