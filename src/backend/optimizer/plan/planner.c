...
#include "../../contrib/fpc_xml/fpc_xml.h"
...
static Node *preprocess_injected_conditions(PlannerInfo *root, Node *clausesel);
				 ...
				 PlannerInfo **subroot)
{
	...
	//@kuntal for selectivity injection
	parse->clausesel = preprocess_injected_conditions(root, (Node *)parse->clausesel);

	if((list_length((List *)root->parse->clausesel)!=list_length(root->parse->selec))) {
		ereport(ERROR,
						(errcode(ERRCODE_INVALID_PARAMETER_VALUE),
						 errmsg("EXPLAIN SELECTIVITY parameters are not same")));
	}
	//end
...
//@kuntal to process injected clauses
static Node*
preprocess_injected_conditions(PlannerInfo *root, Node *clausesel)
{
	return preprocess_expression(root, clausesel, EXPRKIND_QUAL);
}
/*
