...
create_plan(PlannerInfo *root, Path *best_path)
{
	...
	Assert(best_path->cost1 >= 0);
	Assert(best_path->cost2 >= 0);
	Assert(best_path->startup_tuple >= 0);
	Assert(best_path->per_tuple >= 0);

	/* <Added on 12/05/2015 for QUEST> */
	plan->cost1 = best_path->cost1;
	plan->cost2 = best_path->cost2;
	plan->startup_tuple = best_path->startup_tuple;
	plan->per_tuple = best_path->per_tuple;
					  ...
					  bool indexonly)
{
	...
	IndexScan	   *indexScanPlan;	/* <Added on 12/05/2015 for QUEST> */
	IndexOnlyScan  *indexOnlyScanPlan;	/* <Added on 12/05/2015 for QUEST> */
	...
	if (indexonly)
	{
		...
		/* <Added on 12/05/2015 for QUEST> */
		indexOnlyScanPlan = (IndexOnlyScan*)scan_plan;
		indexOnlyScanPlan->loop_count = best_path->loop_count;
		indexOnlyScanPlan->index_correlation = best_path->index_correlation;
		indexOnlyScanPlan->spc_random_page_cost = best_path->spc_random_page_cost;
		indexOnlyScanPlan->qual_arg_cost = best_path->qual_arg_cost;
		indexOnlyScanPlan->per_tuple_cost = best_path->per_tuple_cost;
		indexOnlyScanPlan->num_sa_scans = best_path->num_sa_scans;
		indexOnlyScanPlan->total_table_pages = best_path->total_table_pages;
	...
	else
	{
		scan_plan = (Scan *) make_indexscan(tlist,
											...
											best_path->indexscandir);
		/* <Added on 12/05/2015 for QUEST> */
		indexScanPlan = (IndexScan*)scan_plan;
		indexScanPlan->loop_count = best_path->loop_count;
		indexScanPlan->index_correlation = best_path->index_correlation;
		indexScanPlan->spc_random_page_cost = best_path->spc_random_page_cost;
		indexScanPlan->qual_arg_cost = best_path->qual_arg_cost;
		indexScanPlan->per_tuple_cost = best_path->per_tuple_cost;
		indexScanPlan->num_sa_scans = best_path->num_sa_scans;
		indexScanPlan->total_table_pages = best_path->total_table_pages;
	}
					  ...
					  List **qual, List **indexqual, List **indexECs)
{
	...
	else if (IsA(bitmapqual, IndexPath))
	{
		...
		/* <Added on 12/05/2015 for QUEST> */
		BitmapIndexScan *bitmapIndexScanNode;
		...
		*indexECs = subindexECs;
		...
		/* <Added on 12/05/2015 for QUEST> */
		plan->cost1 = ipath->path.cost1;
		plan->cost2 = ipath->path.cost2;
		plan->startup_tuple = ipath->path.startup_tuple;
		plan->per_tuple = ipath->path.per_tuple;
		bitmapIndexScanNode=(BitmapIndexScan*)plan;
		bitmapIndexScanNode->loop_count=ipath->loop_count;
//		bitmapIndexScanNode->index_pages=ipath->index_pages;
		bitmapIndexScanNode->index_correlation=ipath->index_correlation;
		bitmapIndexScanNode->spc_random_page_cost=ipath->spc_random_page_cost;
		bitmapIndexScanNode->qual_arg_cost=ipath->qual_arg_cost;
		bitmapIndexScanNode->per_tuple_cost=ipath->per_tuple_cost;
		bitmapIndexScanNode->num_sa_scans=ipath->num_sa_scans;
					  ...
					  Plan *inner_plan)
{
	...
	if (best_path->materialize_inner)
	{
		...
		/* <Added on 12/05/2015 for QUEST> */
		best_path->jpath.path.total_cost += cpu_tuple_cost * matplan->plan_rows;
		matplan->cost2 = seq_page_cost;
		matplan->per_tuple = cpu_tuple_cost;
...
copy_path_costsize(Plan *dest, Path *src)
{
	if (src)
	{
		...
		/* enable this line for spilling..but this line is not there in original pgsql9.3.4 source */
//		dest->relids = bms_copy(src->parent->relids);	/* <Added on 12/05/2015 for QUEST> */
	}
	else
	{
		dest->startup_cost = 0;
...
copy_plan_costsize(Plan *dest, Plan *src)
{
	if (src)
	{
		...
		/* enable this line for spilling..but this line is not there in original pgsql9.3.4 source */
//		dest->relids = bms_copy(src->relids);		/* <Added on 12/05/2015 for QUEST> */
	}
	else
	{
		dest->startup_cost = 0;
		...
		/* enable this line for spilling..but this line is not there in original pgsql9.3.4 source */
//		dest->relids = -1;							/* <Added on 12/05/2015 for QUEST> */
	}
}
		  ...
		  double limit_tuples)
{
	...
	/* <Added on 12/05/2015 for QUEST> */
	Assert(sort_path.cost2 >= 0);
	Assert(sort_path.startup_cost >= 0);
	Assert(sort_path.total_cost >= 0);

	/* <Added on 12/05/2015 for QUEST> */
	plan->cost2=sort_path.cost2;
	plan->startup_tuple=sort_path.startup_tuple;
	plan->per_tuple=sort_path.per_tuple;
		 ...
		 Plan *lefttree)
{
	...
	/* <Added on 12/05/2015 for QUEST> */
	Assert(agg_path.cost2 >= 0);
	Assert(agg_path.per_tuple >= 0);
//	plan->cost1=agg_path.cost1;
	plan->cost2=agg_path.cost2;
//	plan->startup_tuple=agg_path.startup_tuple;
	plan->per_tuple=agg_path.per_tuple;
		   ...
		   Plan *lefttree)
{
	...
	/* <Added on 12/05/2015 for QUEST> */
	Assert(group_path.per_tuple >= 0);
//	plan->cost1=group_path.cost1;
//	plan->cost2=group_path.cost2;
//	plan->startup_tuple=group_path.startup_tuple;
	plan->per_tuple = group_path.per_tuple;
...
make_unique(Plan *lefttree, List *distinctList)
{
	...
	/* <Added on 12/05/2015 for QUEST> */
	plan->per_tuple = cpu_operator_cost * numCols;
			...
			Plan *subplan)
{
	...
	QualCost	qual_cost;	/* <Added on 12/05/2015 for QUEST> */
	...
	else
	{
		...
		if (resconstantqual)
		{
			/* <commented on 12/05/2015 for QUEST> */
//			QualCost	qual_cost;
	...
	/* <Added on 12/05/2015 for QUEST> */
	plan->startup_tuple = qual_cost.startup;
	plan->per_tuple = qual_cost.per_tuple;
