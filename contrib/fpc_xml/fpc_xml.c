...
typedef struct Fpc_Join_Hint {
	...
	unsigned char join_materialize_mask;
	Relids join_relids;
...
struct FpcHintState {
	...
	/*initial materialize parameter*/
	int init_materialize_mask;

	/*initial hashagg parameter*/
	int init_hashagg_mask;
...
/* lists used while parsing xml plan tree */
 List *xml_rel_order_list;
 List *xml_seq_scan_rels_list;
 List *xml_idx_scan_rels_list;
 List *xml_idx_only_scan_rels_list;
 List *xml_idx_bmp_scan_rels_list;
 List *xml_joins_list;
List *join_xml_rel_order_list;
/*My code changes----------------------------*/
/*format of indexes----- USE NONE IF THERE IS NO INDEX NAME AVAILABLE.Store the index names of all indexes---*/
List *xml_index_of_idx_scan_list;
List *xml_index_of_idx_only_scan_list;
List *xml_index_of_bmp_heap_scan_list;
...
//changed here
int found_materialize;
int ishashaggenabled;
int fpc_running;
...
static void join_xml_chkfor_plan_node(xmlDocPtr doc, xmlNodePtr cur,int from_join);
...
static void parse_xml(char *file, StringInfoData buf) {
	...
	xml_joins_list = NIL;
	//---------------------
	xml_index_of_idx_scan_list=NIL;
	xml_index_of_idx_only_scan_list=NIL;
	xml_index_of_bmp_heap_scan_list=NIL;

	/*check if there are \n charaters and remove it*/
	...
	for (i = length(xml_idx_scan_rels_list) - 1; i >= 0; i--)
	{
		...
		if(strcmp((char*) list_nth(xml_index_of_idx_scan_list, i),"0")!=0)
		{
			appendStringInfoString(&buf," ");
			appendStringInfoString(&buf,
							(char*) list_nth(xml_index_of_idx_scan_list, i));
		}
		//fprintf(stderr,"came hereidx ");
		appendStringInfoString(&buf, ") ");
	}
	...
	for (i = length(xml_idx_only_scan_rels_list) - 1; i >= 0; i--)
	{
		...
		if(strcmp((char*) list_nth(xml_index_of_idx_only_scan_list, i),"0")!=0)
		{
				appendStringInfoString(&buf," ");
					appendStringInfoString(&buf,
									(char*) list_nth(xml_index_of_idx_only_scan_list, i));
		}
		//fprintf(stderr,"came hereonly ");
		appendStringInfoString(&buf, ") ");
	}
	...
	for (i = length(xml_idx_bmp_scan_rels_list) - 1; i >= 0; i--)
	{
		...
		if(strcmp((char*) list_nth(xml_index_of_bmp_heap_scan_list, i),"0")!=0)
		{
			appendStringInfoString(&buf," ");
					appendStringInfoString(&buf,
									(char*) list_nth(xml_index_of_bmp_heap_scan_list, i));
		}
		//fprintf(stderr,"came herebmp ");
		appendStringInfoString(&buf, ") ");
	}
...
//	printf("\nPlan: %s\n", buf.data);
	...
	xmlCleanupParser();
		...
		if(join_hint_buf.data)
			pfree(join_hint_buf.data);
		if(order_hint_buf.data)
			pfree(order_hint_buf.data);

		 //list_free_deep_malloc(xml_rel_order_list);
		 list_free_deep_malloc(xml_seq_scan_rels_list);
		 list_free_deep_malloc(xml_idx_scan_rels_list);
		 list_free_deep_malloc(xml_idx_only_scan_rels_list);
		 list_free_deep_malloc(xml_idx_bmp_scan_rels_list);
		 list_free_deep_malloc(join_xml_rel_order_list);
		 //must change here-----------------------
		 //list_free_deep_malloc(xml_index_of_idx_scan_list);
		 //list_free_deep_malloc(xml_index_of_idx_only_scan_list);
		 //list_free_deep_malloc(xml_index_of_idx_scan_list);
...
void parse_join_types(xmlDocPtr doc) {
			...
			join_no_in_search++)
	{
		...
		appendStringInfoString(&join_hint_buf, "(");
		if (found_materialize==1)
			appendStringInfoString(&join_hint_buf, "M ");
...
static void join_xml_chkfor_query_node(xmlDocPtr doc) {
	...
	while (cur != NULL) {
		if ((!xmlStrcmp(cur->name, (const xmlChar *) "Query"))) {
			join_xml_chkfor_plan_node(doc, cur,1);
		}
		cur = cur->next;
	}
	return;
...
static void join_xml_chkfor_plan_node(xmlDocPtr doc, xmlNodePtr cur,int from_join) {
	...
	int count=0;
	temp=cur;
	if (from_join==1 && cur_join_no == join_no_in_search+1)
	{
		count=0;
					while (temp!= NULL) {

										if ((!xmlStrcmp(temp->name, (const xmlChar *) "Plan"))) {
											count+=1;
											if(count==2)//first find if any materialization is there if-----
												break;
										}
										temp = temp->next;
								}
					if(count==2)
								...
								temp=temp->xmlChildrenNode;
											...
											while (temp!= NULL) {
													if ((!xmlStrcmp(temp->name, (const xmlChar *) "Node-Type"))) {
														//first find if any materialization is there if-----
														break;
													}
													temp = temp->next;
											}
					...
					if ((!xmlStrcmp(temp->name, (const xmlChar *) "Node-Type"))) {
									...
									if (strcasecmp((char*) "Materialize", (char*) key) == 0)
									{
										found_materialize=1;
									}
...
{
	xmlChar *key, *data,*idx_name;
	//used in case there is Aggregate present.
	xmlNodePtr temp;
//	FILE *fd;
//	fd = fopen("/home/lohitkrishnan/spillBound/dump.text",'a');
	...
	while (cur != NULL)
	{
		if ((!xmlStrcmp(cur->name, (const xmlChar *) "Node-Type")))
		{
			...
			//checking if that was a node of type Aggregate
			if (strcasecmp((char*) "Aggregate", (char*) key) == 0)
			{
				temp=cur;
				while (temp && xmlStrcmp(temp->name, (const xmlChar *) "Strategy"))
									temp = temp->next;
				if (temp && !(xmlStrcmp(temp->name, (const xmlChar *) "Strategy")))
					{
						data = xmlNodeListGetString(doc,temp->xmlChildrenNode, 1);
						if (strcmp("Hashed",(char *)data)==0)
						{
							ishashaggenabled=1;
						}
						else
			...
			/*forgot to scan the index names :( ------------------------------*/
			else if (strcasecmp((char*) "Index Scan", (char*) key) == 0)
			{
				while (cur && xmlStrcmp(cur->name, (const xmlChar *) "Index-Name"))
									{
										//being conservative, because index may not be there :( :O
										if (xmlStrcmp(cur->name, (const xmlChar *) "Alias")==0)
											break;
										cur = cur->next;
									}
				if (cur && !(xmlStrcmp(cur->name, (const xmlChar *) "Index-Name")))
								{
									idx_name = xmlNodeListGetString(doc,
												cur->xmlChildrenNode, 1);

										//xml_rel_order_list = lappend(xml_rel_order_list,(char*) data);
										xml_index_of_idx_scan_list = lappend(
												xml_index_of_idx_scan_list, (char*) idx_name);
										//below lines are not for indxes.
										//appendStringInfoString(&order_hint_buf,(char *)data);
										//appendStringInfoString(&order_hint_buf," ");
										//xmlFree(data);
								}
				...
				{
					xml_index_of_idx_scan_list = lappend(xml_index_of_idx_scan_list, (char *)"0");
				}
				//changed till here------------------------------------------------
			...
			else if (strcasecmp((char*) "Index Only Scan", (char*) key) == 0)
			{
				...
				while (cur && xmlStrcmp(cur->name, (const xmlChar *) "Index-Name"))
								...
								if (cur && !(xmlStrcmp(cur->name, (const xmlChar *) "Index-Name")))
												{
													idx_name = xmlNodeListGetString(doc,
														...
														//xml_rel_order_list = lappend(xml_rel_order_list,(char*) data);
														xml_index_of_idx_only_scan_list = lappend(
																xml_index_of_idx_only_scan_list, (char*) idx_name);
														//below lines are not for indxes.
														//appendStringInfoString(&order_hint_buf,(char *)data);
														//appendStringInfoString(&order_hint_buf," ");
														//xmlFree(data);
								...
								{
									xml_index_of_idx_only_scan_list = lappend(xml_index_of_idx_only_scan_list, "0");
								}
								//changed till here------------------------------------------------
			...
			/* If its a Bitmap Heap Scan scan node, recurse and get relation's alias name */
						...
						{
							//only bitmap indexes requires these type of changes--------
							temp=cur;
							while (temp && xmlStrcmp(temp->name, (const xmlChar *) "Plans"))
												temp = temp->next;
							if (temp && !(xmlStrcmp(temp->name, (const xmlChar *) "Plans")))
											{
												temp=temp->xmlChildrenNode;
												while (temp && xmlStrcmp(temp->name, (const xmlChar *) "Plan"))
												{
													temp=temp->next;
												}
												if (xmlStrcmp(temp->name, (const xmlChar *) "Plan")==0)
												{
													temp=temp->xmlChildrenNode;
													while (temp && xmlStrcmp(temp->name, (const xmlChar *) "Index-Name"))
													{
													//being conservative, because index may not be there :( :O
													if (xmlStrcmp(temp->name, (const xmlChar *) "Alias")==0)
																break;
													temp = temp->next;
													}
													if (temp && !(xmlStrcmp(temp->name, (const xmlChar *) "Index-Name")))
													{
													idx_name = xmlNodeListGetString(doc,
													temp->xmlChildrenNode, 1);
													//xml_rel_order_list = lappend(xml_rel_order_list,(char*) data);
												xml_index_of_bmp_heap_scan_list = lappend(
													xml_index_of_bmp_heap_scan_list, (char*) idx_name);
												//below lines are not for indxes.
										//appendStringInfoString(&order_hint_buf,(char *)data);
															//appendStringInfoString(&order_hint_buf," ");
												//xmlFree(data);
												...
												{
													xml_index_of_bmp_heap_scan_list = lappend(xml_index_of_bmp_heap_scan_list, "0");
												}
...
HintStateCreate(void)
{
	...
	//changed here
	hstate->init_materialize_mask=0;
	//changed here
	hstate->init_hashagg_mask=0;
...
create_hintstate(Query *parse, const char *hints) {
	...
	//fprintf(stderr,hstate->fpc_hint_str);
	/* parse each hint. */
...
Parse_Join_Method_Hint(Fpc_Join_Hint *hint, FpcHintState *hstate,
	...
	{
		materialize_found=1;
		hint->join_materialize_mask = ENABLE_MATERIALIZE;
		str++;
		str++;
	}
	...
	while ((str = parse_value(str, &relname, "relation name", true))
		...
		/*if(strstr(relname,"Materialize_"))
		{
			relname+=12;
			materialize_found=1;
			hint->join_materialize_mask = ENABLE_MATERIALIZE;
...
/*check the hashagg mask and set it*/
static void set_hashagg_config_options(unsigned char enforce_mask,GucContext context) {
	unsigned char mask;

	if (enforce_mask == ENABLE_HASHAGG)
		mask = enforce_mask;
	else
		mask = enforce_mask & current_hint->init_hashagg_mask; /* this enables all */

	SET_CONFIG_OPTION("enable_hashagg", ENABLE_HASHAGG);
}



/* check enforce mask for join hints and set it */
static void set_join_config_options(unsigned char enforce_mask,unsigned char materialize_mask,
		GucContext context) {
	unsigned char mask,mat_mask;
	...
	if (materialize_mask == ENABLE_MATERIALIZE)
			mask=materialize_mask;
		else
			mask=materialize_mask & current_hint->init_materialize_mask;
...
fpc_planner(Query *parse, int cursorOptions, ParamListInfo boundParams) {
	...
	//changed here
	if (enable_material)
		current_hint->init_materialize_mask |= ENABLE_MATERIALIZE;
	//changed here
	if (enable_hashagg)
		current_hint->init_hashagg_mask |= ENABLE_HASHAGG;
	//changing here
		if (ishashaggenabled==1)
		{
			//hoping that it is an fpc query, setting the hashagg configuration parameter here.
			hstate->init_hashagg_mask=1;
			set_hashagg_config_options((unsigned char)(ishashaggenabled),current_hint->context);
		}
		else
		{
			hstate->init_hashagg_mask=0;
			set_hashagg_config_options(DISABLE_HASHAGG,current_hint->context);
		}
	...
	PG_TRY()
		...
		}PG_CATCH();
		{
			...
			fpc_running=0;
			set_hashagg_config_options(ENABLE_ALL_HASHAGG,current_hint->context);
	...
	//added by Srinivas for freeing the global  variables
	//freeing the join and order hint buf
//	if(join_hint_buf.data)
//		pfree(join_hint_buf.data);
//
//	if(order_hint_buf.data)
//		pfree(order_hint_buf.data);
	...
	//now freeing the list variables
//	printf("\nfreeing the variables");
//	 list_free_deep(xml_rel_order_list);
//	 list_free_deep_malloc(xml_seq_scan_rels_list);
//	 list_free_deep_malloc(xml_idx_scan_rels_list);
//	 list_free_deep_malloc(xml_idx_only_scan_rels_list);
//	 list_free_deep_malloc(xml_idx_bmp_scan_rels_list);
...
static bool process_join_hints(FpcHintState *hstate, PlannerInfo *root,
	...
	for (i = 0; i < hstate->num_hints[HINT_TYPE_ORDER]; i++) {
		...
		lhint = leading_hint;           //under construction
	}
	...
	if (fpc_hint_state_enabled(lhint)) {
		set_join_config_options(DISABLE_ALL_JOIN,DISABLE_MATERIALIZE,
				current_hint->context);
		return true;
	}
	return false;
...
make_join_rel_wrapper(PlannerInfo *root, RelOptInfo *rel1,
	...
	bool byepass_joinsearch,notvalid_inner=false;
		...
		{
		if (bms_equal(hint->inner_joinrelids, rel2->relids)||bms_equal(hint->inner_joinrelids, rel1->relids))
				{

					notvalid_inner=false;
				}
				else
				{
					notvalid_inner=true;
	...
	if (hint->inner_nrels == 0)
	{
		...
		set_join_config_options(hint->join_enforce_mask,hint->join_materialize_mask,
				current_hint->context);
...
fpc_join_search(PlannerInfo *root, int levels_needed,
	...
	if (order_hint_enable)
		set_join_config_options(current_hint->init_join_mask,current_hint->init_materialize_mask,
				current_hint->context);
...
void fpc_add_paths_to_joinrel(PlannerInfo *root, RelOptInfo *joinrel,
	...
	if (join_hint && join_hint->inner_nrels != 0)
	{
		...
		if (bms_equal(join_hint->inner_joinrelids, innerrel->relids))
		{
			...
			set_join_config_options(join_hint->join_enforce_mask,join_hint->join_materialize_mask,
					current_hint->context);
			//fprintf(stderr,"\n");
			//for(i=0;i<join_hint->nrels;i++)
			//{
				//fprintf(stderr,"%s ",join_hint->rel_names[i]);
			//}
			//fprintf(stderr,"the materialize is-------%d or %c\n",join_hint->join_materialize_mask,join_hint->join_materialize_mask);
